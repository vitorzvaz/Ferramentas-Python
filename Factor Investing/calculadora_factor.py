...
class backtest_indicators():
    ...
    def pegando_dados(self):
        ...
        cotacoes = pd.read_parquet(os.path.join(self.caminho_dados, 'cotacoes.parquet'))
        ...
        volume_mediano = pd.read_parquet(os.path.join(self.caminho_dados, 'volume_mediano.parquet'))
        ...
        for carteira in self.indicadores.values():
            ...
            for indicador in indicadores.keys():
                ...
                if indicador in lista_indicadores_sem_rep:
                    ...
                    lendo_indicador = pd.read_parquet(os.path.join(self.caminho_dados, f'{indicador}.parquet')) 
    ...
    def criando_carteiras(self):
        ...
        carteira_por_periodo.to_excel('carteira_por_periodo.xlsx')
        carteira_por_periodo = carteira_por_periodo[['data', 'ticker', 'peso']]
    ...
    def make_report(self):
        ...
        MakeReportResult(df_trades=self.df_retornos, df_carteiras=self.carteira_por_periodo, nome_arquivo=self.nome_arquivo,
                         caminho_imagens=self.caminho_imagens, caminho_dados=self.caminho_dados)
...
if __name__ == "__main__":
    ...
    dicionario_carteira = {
        'carteira1': {
                'indicadores': {
                    'EBITDA_EV': {'caracteristica': 'decrescente'},
                    'ValorDeMercado': {'caracteristica': 'crescente'},
                    'price_range': {'caracteristica': 'decrescente'},
                },
                'peso': 1,
        },
    }
    ...
    nome_pdf = nome_pdf + str(balanceamento) + '_' + str(filtro_liquidez) + "M_" + str(numero_ativos) + ".pdf"

    backtest = backtest_indicators(data_final="2024-09-01", data_inicial= '2011-01-01', filtro_liquidez=(filtro_liquidez * 2000000), balanceamento=balanceamento, 
